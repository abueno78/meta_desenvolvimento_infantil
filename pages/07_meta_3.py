...
st.markdown("## Meta 3 - Registros de Peso e Altura")
st.info("Ter pelo menos 09 (nove) registros simultâneos de peso e altura até os dois anos de vida.")
...
try:
    ...
    if df is not None and len(df) > 0:
        ...
        # Métricas Meta 3
        st.markdown("### Métricas - Meta 3")
        
        total = len(df)
        conformes = df["Indicador_C"].sum() if total > 0 else 0
        ...
        with col1:
            st.plotly_chart(
                criar_grafico_conformidade_por_variavel(df, "Sexo", "Indicador_C"),
                use_container_width=True
            )
        with col2:
            st.plotly_chart(
                criar_grafico_conformidade_por_variavel(df, "Raça/cor", "Indicador_C"),
                use_container_width=True
            )
        ...
        with col1:
            st.plotly_chart(
                criar_grafico_conformidade_por_variavel(df, "Beneficiário do programa Bolsa Família", "Indicador_C"),
                use_container_width=True
            )
        with col2:
            st.plotly_chart(
                criar_grafico_conformidade_por_variavel(df, "Microárea", "Indicador_C"),
                use_container_width=True
            )
        ...
        colunas_exibicao = ["Nome", "CPF", "CNS", "Telefone celular", "Telefone residencial", "Telefone de contato", "Indicador_C"]
        colunas_exibicao = [c for c in colunas_exibicao if c in df.columns]
        
        df_tabela = df[colunas_exibicao].copy()
        
        # Renomear coluna Indicador_C para "C"
        df_tabela = df_tabela.rename(columns={"Indicador_C": "C"})
        
        # Converter booleanos para símbolos
        df_tabela["C"] = df_tabela["C"].apply(lambda x: "✓" if x else "✗")
